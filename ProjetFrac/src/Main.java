...
public class Main {
	...
	public Main(Calcul defaultCalcul) throws InterruptedException, IOException {
		if (chooseView()) {
			Calcul.Builder b = maker();
			this.calcul = b.build();
	...
	//	Complex x = selectJulia();
		...
		return new Calcul.Builder().step(step).plan(planSize).size(size,size);
	}
	...
	public void genButton(double plan, int size, double step, Complex c, int colors, Complex newOrigin) {
		Calcul calc = new Calcul.Builder().plan(plan).size(size, size).step(step).comp(c).col(colors)
	 ...
	 * View(g.image); <<<<<<< HEAD ======= public BufferedImage generate() { Calcul
	 * c = new Calcul.Builder().plan(1.0).size(500, 500).build(); ImageGenerator g =
